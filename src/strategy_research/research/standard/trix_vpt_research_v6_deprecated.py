...
class TrixVptResearch(PandasBacktestingBase):
    ...
    def compute_indicators(self,
        ...
        daily_signal_bar_df = daily_signal_bar_df.assign(
            ...
            channel_compression_20=lambda x: x["channel_compression"].rolling(60).quantile(0.3),
        ...
        daily_exec_bar_df = daily_signal_bar_df.assign(
            atr_14=lambda x: x[f"atr_{self.atr_period}"].shift(1),
    ...
    def hybrid_backtest(self,
        ...
        daily_prev_close_arr = bar_exec_df_copy['daily_prev_close'].to_numpy()
        ...
        for i in range(bar_exec_df_copy_length):
            ...
            prev_daily_close = daily_prev_close_arr[i]
            ...
            if cur_position is None or cur_position.volume == 0:
                ...
                if (
                        ...
                        and cur_price > (cur_channel_high + 0.3 * cur_daily_atr_14)
                        and cur_intraday_bar_index >= 30
                        and cur_return_today_open > 0
                ):
                    """多头开仓"""
                    stop_loss = max(cur_channel_low, cur_price - cur_daily_atr_14 * self.stop_loss_multiplier)
                    take_profit = cur_price + cur_daily_atr_14 * self.take_profit_multiplier
                    self.open_position(
                        self.symbol,
                        cur_price,
                        ...
                        cur_is_channel_compression
                        and cur_price < (cur_channel_low - 0.3 *cur_daily_atr_14)
                        and cur_intraday_bar_index >= 30
                        and cur_return_today_open < 0
                ):
                    """空头开仓"""
                    stop_loss = min(cur_channel_high, cur_price + cur_daily_atr_14 * self.stop_loss_multiplier)
                    take_profit = cur_price - cur_daily_atr_14 * self.take_profit_multiplier
                    self.open_position(
                        self.symbol,
                        cur_price,
                ...
                if cur_position.direction == BacktestingDirection.LONG:
                    ...
                    #     exit_reason = '止盈'
                    elif cur_price < (prev_daily_close - 2 * cur_daily_atr_14):
                        close_reason = '下跌超过ATR阈值'
                    elif (
