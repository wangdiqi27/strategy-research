...
class TrixVptResearch(PandasBacktestingBase):
    ...
    def compute_indicators(self,
        ...
        daily_exec_bar_df = daily_signal_bar_df.assign(
            ...
            vpt_hist=lambda x: x["vpt_hist"].shift(1),
        ...
        minute_exec_bar_df = minute_exec_bar_df.assign(
            return_prev_close=lambda x: (x["close"] / x["daily_prev_close"] - 1),
            return_prev_high=lambda x: (x["close"] / x["daily_prev_high"] - 1),
            return_prev_low=lambda x: (x["close"] / x["daily_prev_low"] - 1),
            return_today_open=lambda x: (x["close"] / x["daily_today_open"] - 1),
            above_prev_high=lambda x: (x["close"] >= x["daily_prev_high"]),
            below_prev_low=lambda x: (x["close"] <= x["daily_prev_low"]),
            break_prev_high=lambda x: (x["high"] > x["daily_prev_high"]),
            break_prev_low=lambda x: (x["low"] < x["daily_prev_low"]),
        )
        ...
        minute_exec_bar_df = minute_exec_bar_df.assign(
            ...
            prev_intraday_high=group_df["high"].shift(1).cummax(),
            prev_intraday_low=group_df["low"].shift(1).cummin(),
            prev_intraday_rolling_high_30=group_df["high"].transform(
                lambda x: x.shift(1).rolling(30, min_periods=30).max()),
            prev_intraday_rolling_low_30=group_df["low"].transform(
    ...
    def hybrid_backtest(self,
        ...
        minute_return_prev_close_arr = bar_exec_df_copy['return_prev_close'].to_numpy()
        minute_return_today_open_arr = bar_exec_df_copy['return_today_open'].to_numpy()
        ...
        daily_vpt_arr = bar_exec_df_copy['daily_vpt'].to_numpy()
        daily_vpt_hist_arr = bar_exec_df_copy['daily_vpt_hist'].to_numpy()
        ...
        for i in range(bar_exec_df_copy_length):
            ...
            cur_return_prev_close = minute_return_prev_close_arr[i]
            cur_return_today_open = minute_return_today_open_arr[i]
            ...
            if cur_position is None or cur_position.volume == 0:
                ...
                if (
                    ...
                    # stop_loss = max(daily_channel_low_arr[i], cur_price - daily_atr_arr[i] * self.stop_loss_multiplier)
                    ...
                    """空头开仓"""
                    # stop_loss = min(daily_channel_high_arr[i], cur_price + daily_atr_arr[i] * self.stop_loss_multiplier)
                    stop_loss = cur_price + daily_atr_arr[i] * self.stop_loss_multiplier
                    take_profit = cur_price - daily_atr_arr[i] * self.take_profit_multiplier
                    self.open_position(
                        self.symbol,
