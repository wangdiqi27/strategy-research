...
class TrixVptResearch(PandasBacktestingBase):
    ...
    def hybrid_backtest(self,
        ...
        daily_atr_14_arr = bar_exec_df_copy['daily_atr'].to_numpy()
        ...
        for i in range(bar_exec_df_copy_length):
            ...
            if cur_position is None or cur_position.volume == 0:
                ...
                if (
                        ...
                        and cur_price > cur_channel_high
                        and cur_intraday_bar_index >= 30
                        and cur_return_today_open > 0
                ):
                    """多头开仓"""
                    stop_loss = max(cur_channel_low, cur_price - daily_atr_14_arr[i] * self.stop_loss_multiplier)
                    take_profit = cur_price + daily_atr_14_arr[i] * self.take_profit_multiplier
                    self.open_position(
                        self.symbol,
                        cur_price,
                        ...
                        cur_is_channel_compression
                        and cur_price < cur_channel_low
                        and cur_intraday_bar_index >= 30
                        and cur_return_today_open < 0
                ):
                    """空头开仓"""
                    stop_loss = min(cur_channel_high, cur_price + daily_atr_14_arr[i] * self.stop_loss_multiplier)
                    take_profit = cur_price - daily_atr_14_arr[i] * self.take_profit_multiplier
                    self.open_position(
                        self.symbol,
                        cur_price,
